...
Node* CalendarTree::divideNode(Node* p, Node *child)
{
	...
	newP->fixParent();
	//	newP->fixMins();
	if (p->getParnet() == nullptr || newP->getParnet() == nullptr)
	{
		Node *newRoot = nullptr;
		Node *mid = root->getRightChild();

		if (p->getmin(1) < mid->getmin(1) && mid->getmin(1) < newP->getmin(1))
			newRoot = new Node(p->getmin(1), mid->getmin(1), newP->getmin(1), nullptr, p, mid, newP);
		else if (p->getmin(1) < newP->getmin(1) && newP->getmin(1) < mid->getmin(1))
			newRoot = new Node(p->getmin(1), newP->getmin(1), mid->getmin(1), nullptr, p, newP, mid);
		else if (mid->getmin(1) < newP->getmin(1) && newP->getmin(1) < p->getmin(1))
			newRoot = new Node(mid->getmin(1), newP->getmin(1), p->getmin(1), nullptr, mid, newP, p);
		else if (mid->getmin(1) < p->getmin(1) && p->getmin(1) < newP->getmin(1))
			newRoot = new Node(mid->getmin(1), p->getmin(1), newP->getmin(1), nullptr, mid, p, newP);
		else if (newP->getmin(1) < p->getmin(1) && p->getmin(1) < mid->getmin(1))
			newRoot = new Node(newP->getmin(1), p->getmin(1), mid->getmin(1), nullptr, newP, p, mid);
		else if (newP->getmin(1) < mid->getmin(1) && mid->getmin(1) < p->getmin(1))
			newRoot = new Node(newP->getmin(1), mid->getmin(1), p->getmin(1), nullptr, newP, mid, p);
